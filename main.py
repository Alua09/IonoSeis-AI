...
with tab4:
    ...
    st.markdown("""
    Наша система использует модель **литосферно-ионосферного взаимодействия (LIS)**:
    1. **Физика процесса:** Тектоническое напряжение вызывает микроразряды и выброс радона. Газ ионизирует воздух, создавая электрическое поле, искажающее ионосферу.
    2. **Интерпретация:** Мы сравниваем текущую плотность ионосферы (VTEC) с динамической нормой. 
    3. **Зачем карта?** Карта на панели мониторинга нужна для точной локализации региона и визуального контроля радиуса поиска землетрясений (500 км).
    4. **Фильтрация:** Чтобы исключить влияние магнитных бурь, мы используем данные **NOAA** (Kp-индекс). Если Kp > 4, система понимает, что возмущение вызвано Солнцем, а не недрами Земли.
    """)
    st.latex(r"Z = \frac{VTEC_{obs} - VTEC_{norm}}{\sigma}")s