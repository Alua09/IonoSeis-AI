        ...
        with st.container(border=True):
            ...
            if power > 5.0:
                c3.warning(f"⚠️ SPECTRAL ANOMALY: {power:.1f}", icon="〰️",
                           help="Анализ частот (FFT) выявляет скрытые колебания, которые могут предшествовать сейсмическим событиям.")
            else:
...
with tab4:
    ...
    * **Спектральный анализ (FFT):** Представьте, что ионосфера — это гигантская струна. Обычный анализ видит, как сильно она натянута, а спектральный анализ «слушает» её вибрации. Тектонические процессы перед землетрясением создают специфический «низкочастотный гул» (микроколебания). FFT позволяет нам выявить этот гул и заметить предвестник еще до того, как общая плотность (VTEC) выйдет за рамки нормы.
    ...
    * $VTEC_{norm}$ — «нормальное» состояние.
    * $\sigma$ — статистическая погрешность (шум).

    **3. Фильтр помех**
    Система применяет фильтры:
    * **Контроль Kp-индекса:** Если Kp > 4, ионосферу «трясёт» от солнечного ветра, а не от тектоники.
    * **Анализ радиуса:** Проверяем только события в радиусе 500 км.
    * **Синхронизация с USGS:** Сопоставляем наши сигналы с реальными данными о землетрясениях.
    """)