...
with tab1:
    for city, (lat, lon, offset) in CITIES.items():
        ...
        with st.container(border=True):
            ...
            # Убрали help из success/error, так как старые версии Streamlit их не поддерживают
...
with tab4:
    ...
    Наша система базируется на концепции **Литосферно-Ионосферного Взаимодействия (LIS)**. Мы рассматриваем ионосферу как датчик, реагирующий на тектонические процессы.

    1. **Физический механизм:** Перед сильным землетрясением в коре возникают микротрещины. Высвобождается газ **Радон**, который ионизирует приземный слой атмосферы.
    2. **Электрический отклик:** Ионы устремляются вверх и искажают плотность электронов (**VTEC**).
    3. **Математическая модель:** Мы используем Z-оценку:
    """)
    st.latex(r"Z = \frac{VTEC_{obs} - VTEC_{norm}}{\sigma}")
    st.markdown("""
    * *Пояснение:* Если значение Z выходит за рамки, система фиксирует аномалию.
    4. **Анализ волатильности:** Рост «хаотичности» данных VTEC — признак накопления напряжения в недрах Земли.
    5. **Фильтрация:** Используем Kp-индекс для исключения влияния магнитных бурь (Солнца) на данные.

    > **Вывод:** Проект является статистическим инструментом, позволяющим эффективно отделять случайные атмосферные помехи от геофизических сигналов-предвестников.
    """)