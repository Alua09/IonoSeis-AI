...
with tab4:
    ...
    **2. Математический метод: поиск аномалий**
    Чтобы понять, что ионосфера «волнуется» именно из-за тектоники, мы используем два метода:
    * **Z-оценка:** Показывает отклонение от нормы.
    * **Спектральный анализ (FFT):** Представьте, что ионосфера — это гигантская натянутая струна. Обычный анализ (Z-оценка) видит, как сильно она натянута. А спектральный анализ (FFT) «слушает» её вибрации. Тектонические процессы перед землетрясением создают специфический «низкочастотный гул» (микроколебания). FFT позволяет нам выявить этот гул и заметить предвестник еще до того, как общая плотность (VTEC) выйдет за рамки нормы.
