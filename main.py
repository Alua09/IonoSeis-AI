...
with tab4:
    ...
    **2. Математический метод: поиск аномалий**
    Мы используем два ключевых инструмента анализа:
    * **Z-оценка:** Вычисляем, насколько текущее состояние ионосферы (VTEC) отличается от «нормы» для этого времени суток.
    * **Спектральный анализ (FFT):** Анализируем частотную структуру данных, чтобы выделить низкочастотные циклы, характерные для предвестников сейсмической активности.
    ...
    * **Анализ радиуса:** Проверяем только те события, которые происходят в радиусе 500 км от региона.
