...
withwith tab4:
    ...
    Наша система основана на гипотезе **Литосферно-Ионосферного Взаимодействия (LIS)**. Мы анализируем связь между тектоническими процессами в недрах Земли и состоянием ионосферы.
    ...
    1. **Физический механизм:** Перед сейсмическим событием в земной коре из-за деформации пород возникают пьезоэлектрические эффекты и выбросы радона. 
       - *Подсказка:* Радон ионизирует воздух, создавая поток заряженных частиц, которые «пробивают» атмосферу и достигают ионосферы.
    2. **Ионосферный отклик:** Эти процессы меняют концентрацию электронов. Мы фиксируем это как аномальные скачки параметров **VTEC** (общее электронное содержание).
    3. **Математическая модель (Z-оценка):** Для поиска аномалий мы используем формулу:
    """)

    st.latex(r"Z = \frac{VTEC_{obs} - VTEC_{norm}}{\sigma}")

    st.markdown("""
    - *Подсказка:* $VTEC_{obs}$ — текущее значение; $VTEC_{norm}$ — «нормальная» плотность электронов; $\sigma$ — фоновый шум. Если $Z$ выходит за рамки, система фиксирует аномалию.
    
    4. **Анализ волатильности:** Мы смотрим на «стабильность» данных. 
       - *Подсказка:* Если VTEC стал «нервным» (резкие скачки), это признак нарастающего напряжения в литосфере. Спокойные данные — это просто помехи, на них мы не реагируем.
       
    5. **Фильтрация шумов:** Данные о геомагнитной активности (**Kp-индекс**) от NOAA позволяют отличить «космическую погоду» (магнитные бури) от тектонических предвестников. Если на Солнце вспышка — система понимает, что VTEC изменился из-за неё, а не из-за землетрясения.
    """)

    st.info("💡 **Вывод:** Система не просто «видит» данные, она использует статистический фильтр, чтобы отсеять хаотичный шум и оставить только значимые геофизические сигналы.")