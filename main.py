...
def load_regional_ionosphere_data(selected_station, target_date):
    ...
    if target_date.day == 28 and target_date.month == 10:  
        # Искусственный день экстремальной магнитной бури в истории для демонстрации фильтра шума
    ...
    # Базовый ионосферный фон с учетом влияния текущей космической погоды (устраняет ложный сдвиг)
    ...
    # Фактический измеряемый трек станций GNSS
    df["Raw_VTEC"] = df["Base_VTEC"] + np.random.normal(0, loc["local_noise"], 24)

    # --- 3. ИИ-ЭМУЛЯТОР СЕЙСМОТЕКТОНИЧЕСКИХ ПРЕКУРСОРОВ (Для демонстрации) ---
    if loc["is_seismic"]:
        if target_date.day == 15:
            # ИДЕАЛЬНЫЙ КРАСНЫЙ КОД: мощный всплеск плазмы на разломе с 12:00 до 16:00
            df.loc[12:16, "Raw_VTEC"] += 4.5
        elif target_date.day in [5, 20]:
            # ЖЕЛТЫЙ КОД: локальное деформационное напряжение коры
...
# ИСПРАВЛЕННЫЙ КАЛЕНДАРЬ: Жесткое приведение типов через встроенные объекты datetime
...
with st.spinner("Синхронизация с базами данных NASA/NOAA..."):
    # Переводим в стандартный объект даты для кэша
...
if data is not None:
    ...
    # Определение результирующего суточного вердикта ИИ для приборной панели
    ...
    # Верхний график: Электронный спектр плазмы VTEC
    ...
    # Отображение точек прекурсоров при превышении математического порога
    ...
    # Нижний график: Геомагнитный индекс Kp
