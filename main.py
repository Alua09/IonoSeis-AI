...
with tab4:
    ...
    st.markdown("")
    st.markdown(
        "Наша система работает на базе теории **литосферно-ионосферного взаимодействия (LIS)**. Когда в земной коре растет напряжение, возникают микродеформации и выброс газов (радона), которые меняют плотность ионосферы.")
    st.latex(r"Z = \frac{VTEC_{obs} - VTEC_{norm}}{\sigma}")
    st.markdown("Фильтрация помех осуществляется через Kp-индекс (солнечная активность) и сопоставление с USGS.")