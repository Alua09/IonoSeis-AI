...
def live_vtec_monitor(f107):
    for city, (lat, lon, offset) in CITIES.items():
        ...
        with st.container(border=True):
            ...
            # Help оставлен только там, где он работает по API (в metric)
            d_color = "inverse" if abs(z) > 1.8 else "normal"
            c1.metric("Текущий VTEC", f"{val:.1f} TECU", f"{z:+.1f}σ", delta_color=d_color,
                      help="Z-score > 1.8: статистически значимое отклонение ионосферы от нормы.")
...
col1.metric("Геомагнитный Kp-индекс", kp, help="Индекс активности: Kp > 4 может вызывать ложные аномалии.")
col2.metric("Солнечный поток F10.7", f107, help="Базовый показатель солнечной активности.")
col3.metric("Время UTC", datetime.now(timezone.utc).strftime('%H:%M:%S'), help="Синхронизированное системное время.")
    ...
    if st.session_state.alerts:
        for alert in reversed(st.session_state.alerts):
            with st.expander(f"🔴 {alert['time']} | {alert['city']} | Z={alert['val']}σ"):
                st.write(f"**Анализ:** {alert['msg']}. Проверьте Kp-индекс для исключения геомагнитных шумов.")
    else:
...
with tab4:
    ...
    *Критерии успеха для жюри:*
    1. **Z > 1.8:** Высокая вероятность аномалии.
    2. **Kp < 4:** Исключение влияния солнечного ветра.
    3. **Локализация:** Сопоставление с данными USGS. 
    """)