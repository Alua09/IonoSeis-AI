...
c1.metric("Kp-индекс", kp,
          help="Геомагнитный индекс (0-9). > 4 указывает на магнитную бурю, что может давать ложные сигналы.")
c2.metric("Поток F10.7", f107, help="Солнечный поток: ключевой показатель уровня ионизации верхних слоев атмосферы.")
c3.metric("Время UTC", datetime.now(timezone.utc).strftime('%H:%M:%S'),
          help="Время по Гринвичу для точной синхронизации данных.")
    ...
    for city, (lat, lon, offset) in CITIES.items():
        ...
        with st.container(border=True):
            ...
            sub1.metric("VTEC", f"{val:.1f} TECU", f"{z:+.1f}σ",
                        help="Общее электронное содержание (VTEC). Показывает плотность электронов в ионосфере.")

            if abs(z) <= 2.5 and volatility < 0.8:
                sub2.success("СТАТУС: НОРМА",
                             help="Показатели стабильны. В ионосфере нет признаков тектонического напряжения.")
            else:
                sub2.error("СТАТУС: АНОМАЛИЯ",
                           help="Выявлено критическое отклонение (Z-оценка) или рост волатильности!")

            sub3.info("Сейсмика: OK",
                      help="Мониторинг USGS: в радиусе 500 км нет значимой сейсмической активности выше 3.0 баллов.")
...
with tab4:
    ...
    Наша система базируется на гипотезе **Литосферно-Ионосферного Взаимодействия (LIS)**. Мы рассматриваем ионосферу как датчик, реагирующий на тектонические процессы.

    1. **Физический механизм:** Перед землетрясением в земной коре из-за высокого давления возникают микротрещины. В этот момент высвобождается радиоактивный газ **Радон**. 
    2. **Электрический отклик:** Радон ионизирует воздух у поверхности Земли. Создаются потоки заряженных частиц (ионов), которые под действием электрических полей устремляются вверх, в ионосферу.
    3. **VTEC и аномалии:** На высоте 100–300 км эти частицы меняют плотность электронов. Мы называем это **VTEC** (Total Electron Content).
    4. **Математическая модель:** Для поиска аномалий мы используем Z-оценку:
    """)
    st.latex(r"Z = \frac{VTEC_{obs} - VTEC_{norm}}{\sigma}")
    st.markdown("""
    * *Где:* $VTEC_{obs}$ — текущее значение, $VTEC_{norm}$ — статистическая норма, $\sigma$ — показатель фонового «шума». Если $Z > 2.5$, мы фиксируем аномалию.

    5. **Анализ волатильности:** Рост «нервозности» (скачков) VTEC указывает на накопление напряжения в недрах. Мы отсекаем магнитные бури с помощью **Kp-индекса**, чтобы исключить влияние Солнца.

    > **Вывод:** IonoSeis AI не просто "видит" данные, он использует статистический фильтр, чтобы отсеять случайные помехи и оставить только те сигналы, которые могут быть предвестниками землетрясений.
    """)