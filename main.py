...
c1.metric("Kp-индекс", kp,
          help="Индекс геомагнитной активности: >4 указывает на магнитную бурю, влияющую на ионосферу.")
c2.metric("Поток F10.7", f107,
          help="Солнечная активность: определяет базовый уровень ионизации верхних слоев атмосферы.")
c3.metric("Время UTC", datetime.now(timezone.utc).strftime('%H:%M:%S'),
          help="Мировое время для синхронизации с глобальными станциями.")
        ...
        for city, (lat, lon, offset) in CITIES.items():
            ...
            with st.container(border=True):
                ...
                col1.metric("VTEC", f"{val:.1f} TECU", f"{z:+.1f}σ",
                            help="Плотность электронов и отклонение от суточной нормы.")
...
with tab3:
    for city, (lat, lon, _) in CITIES.items():
        ...
        for q in get_recent_quakes(lat, lon):
            p = q['properties']
            dt = datetime.fromtimestamp(p['time'] / 1000).strftime('%d.%m %H:%M')
            st.write(f"📅 **{dt}** | {'🚨 ' + str(p['mag']) if p['mag'] >= 5.0 else p['mag']} M | {p['place']}")

with tab4:
    st.subheader("🧪 Научно-методологическая база")

    st.markdown("""
    ### Литосферно-ионосферное взаимодействие (LIS)
    Наша система базируется на гипотезе, что накопление тектонического напряжения приводит к электромагнитным возмущениям.

    1. **Физический механизм:** При деформации горных пород возникают пьезоэлектрические эффекты и выделение ионизированных газов (радона). Эти факторы создают слабые электрические поля, проникающие в ионосферу и меняющие концентрацию заряженных частиц.
    2. **Математическая модель:** - $VTEC_{obs}$ — наблюдаемая плотность электронной концентрации.
       - $VTEC_{norm}$ — вычисляется на основе модели солнечной активности ($F10.7$) и суточного ритма.
       - $Z = \frac{VTEC_{obs} - VTEC_{norm}}{\sigma}$ — Z-оценка (стандартное отклонение), позволяющая отделить шум от предвестника.
    3. **Фильтрация:** Использование данных NOAA (Kp-индекс) позволяет исключить ложные срабатывания, связанные с магнитными бурями.
    4. **Зачем нужна карта?** Карта визуализирует регионы мониторинга, подтверждая, что анализируемые данные относятся к конкретным тектонически активным зонам в радиусе 500 км от эпицентров.
    """)