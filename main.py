...
# Элементы управления, которые всегда будут на экране
tab1, tab2, tab3 = st.tabs(["🟢 Live-мониторинг", "📂 Сейсмо-архив", "📊 Анализ нормы VTEC"])

with tab1:
    # Очищаем данные из USGS
    try:
    ...
    for city, (lat, lon, offset) in CITIES.items():
        st.markdown("---")
        # Локальное время для каждого города
        ...
        val = get_diurnal_trend(hour, lat, 150.0) + np.random.normal(0, 0.5)
        ...
        if nearby:
            col3.error(f"🚨 {nearby[0]['properties']['mag']}M | {nearby[0]['properties']['place']}")
        else:
...
with tab2:
    ...
    if st.session_state.archive_results:
        ...
        st.info("Выберите параметры и нажмите кнопку для загрузки архива.")
...
with tab3:
    ...
    st.info(f"Ожидаемый VTEC для {c} в {h}:00 — **{norm} TECU**.")