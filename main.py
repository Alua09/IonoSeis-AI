...
with tab4:
    ...
    st.markdown("""
    Наша система работает на базе теории **литосферно-ионосферного взаимодействия (LIS)**. Мы исходим из того, что тектонические процессы в глубине планеты создают «эхо», которое улавливает ионосфера.

    **1. Суть метода:**
    Перед землетрясением в коре из-за нагрузки возникают микродеформации, выбросы газов и электрических полей. Эти возмущения меняют плотность электронов в ионосфере.

    **2. Математика:**
    Мы считаем **Z-score** — показатель того, насколько сильно текущие данные отличаются от обычной нормы:
    """)
    st.latex(r"Z = \frac{VTEC_{obs} - VTEC_{norm}}{\sigma}")
    st.markdown("""
    **3. Фильтрация помех:**
    Чтобы не спутать землетрясение с солнечной активностью, система фильтрует данные по Kp-индексу (магнитные бури). Если Солнце «спокойно», а ионосфера — нет, вероятность тектонического предвестника растет.
    """)